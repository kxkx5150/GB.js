...
class GBC_sound {
  constructor() {
    ...
    this.nodes = [
      ...
      },
      {
        ...
        bitPeriod: 1,
        freq: (bits4, bits3) => {
          this.nodes[4].bitPeriod =
            this.audioCtx.sampleRate * this.snd4bit4[bits4] * this.snd4bit3[bits3];
        },
      },
    ];
    this.reset();
  ...
  setSound3Waveform() {
    ...
    this.nodes[3].oscillator.setPeriodicWave(
      this.audioCtx.createPeriodicWave(
        real.slice(0, this.FFTsize / 2),
        imag.slice(0, this.FFTsize / 2),
        {
